		...
		if (is_string ($key)) {
			...
			switch ($slug) {
				...
				case 'votes_score':
					...
					if ($min == $max) return 0;
					return floor (self::ROUND3_MIN_SCORE + (self::ROUND3_MAX_SCORE - self::ROUND3_MIN_SCORE) * ($votes[$this->ID] - $min) / ($max - $min));
					break;
				case 'score':
					...
					if (!empty ($questions))
						foreach ($questions as $character_id => $character_q) {
							...
							foreach ($character_q as $question) {
								...
								$round1_score += isset ($answer->player_score) ? $answer->player_score : 0;
								}
							}
					...
					if (!$products->is ('empty')) {
						...
						foreach ($products->get () as $product) {
							...
							$perceived_value = $this->compute ($product, 'rtwo_perceived_value');
							$desired_value = $this->compute ($product, 'desired_value');

							if (!empty ($qualities))
								foreach ($qualities as $quality_slug => $quality_data) {
									$round2_score += $desired_value[$quality_slug] ? floor (100 * abs ($perceived_value[$quality_slug] / $desired_value[$quality_slug] - 1)) : 25;
									$count ++;
									}
							}
						$round2_score /= $count;
						$round2_score = 25 - floor ($round2_score);
...
*/
					...
					switch ($negotiation_step) {
						case SD_Game::NEGOTIATION_0:
							...
							if (!is_null ($opts) && isset ($all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_1]))
								$quotations = $all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_1];
							break;
						case SD_Game::NEGOTIATION_1:
							$quotations = $all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_1];
							if (!is_null ($opts) && isset ($all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_2]))
								$quotations = $all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_2];
							break;
						case SD_Game::NEGOTIATION_2:
							$quotations = $all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_2];
							if (!is_null ($opts) && isset ($all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_3]))
								...
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (!empty ($quotation_data['features']) && is_array ($quotation_data['features'])) {
										foreach ($quotation_data['features'] as $feature_id) {
											...
											$computed[$quality_slug] += 0.01 * $feature->get ('cost') * $qualities[$quality_slug]['unit_cost'];
											}
										}
									}
						}
							...
							foreach ($quotations as $quotation_id => $quotation_data)
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (isset ($quotation_data['location'])) {
										...
										0.01 * $location->get ('delivery_cost') * $qualities[$quality_slug]['unit_cost'] + 
										(($location->get ('delivery_time') - $quotation_data['delivery_term']) > 0 ?
											0.01 * ($location->get ('delivery_time') - $quotation_data['delivery_term']) * $location->get ('day_saved_cost') * $qualities[$quality_slug]['unit_cost'] :
											0);
										}
									}
						}
							...
							foreach ($quotations as $quotation_id => $quotation_data)
								if ($quotation_data['product'] == $key-> get() && $quotation_data['quality'] == $quality_slug) {
									if (isset ($quotation_data['warranty']) && $quotation_data['warranty'] != 'none') {
										...
										$computed[$quality_slug] += $warranty->get ('cost') * $qualities[$quality_slug]['unit_cost'] * 0.01;
										}
									}
						}
		...
		if (is_string ($key)) {
			...
			switch ($key) {
				...
				case 'perceived_quantities':
					...
					$perceived_value = $this->compute ('perceived_value');
					...
					foreach ($products->get () as $product) {
						...
						foreach ($units_quoted as $quality_slug => $value) {
							$computed[$product->get ()][$quality_slug] = [
								...
								$perceived_value[$product->get ()][$quality_slug] * $qualities[$quality_slug]['desired_quantity']
								];
							}
						} 
					break;
...
Price/{unit_type}: {price} {currency}   Payment term: {payment_term} days.'/*]*/, $quotation, FALSE);
	...
	public function report ($html = TRUE, $echo = TRUE) {
		if ($html) {
			...
			$perceived_values = [];
			foreach ($players->get () as $player){
				$perceived_values[$player->get()] = $player->compute ('perceived_value');
				}
			foreach ($products->get () as $product) {
				$qualities = $product->get ('quality');
				foreach ($qualities as $quality_slug => $quality_data) {
					...
					foreach ($players->get () as $player) {
						...
						$charts[$product->get ()][$quality_slug][] = [
							$player->get ('name') . "\n" . 
								...
								' PV' . ': ' . $perceived_values[$player->get ()][$product->get ()][$quality_slug] . $game->scenario ('currency'),
...
#				foreach ($all_states[SD_Game::ROUND4_BEGIN]['data']['negotiation'][SD_Game::NEGOTIATION_3] as $quotation_id => $quotation)
		...
		<table class="table table-striped table-hover">
			...
			<tbody>
				<tr>
					<td>' . $this->get ('score', 'round1') . '</td>
					<td>' . $this->get ('score', 'round2') . '</td>
					<td>' . $this->get ('score', 'round3') . '</td>
					<td>' . $score . '</td>
				</tr>
			</tbody>
		</table>
